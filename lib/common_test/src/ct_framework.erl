...
error_notification(Mod,Func,Args,{Error,Loc}) ->
    ct_event:notify(#event{name=tc_error, node=node(), data={Mod, Func, Args, {Error, Loc}}}),
    ok.
...
report(What,Data) ->
	...
	tests_start ->
	    ok;
        tests_collected ->
            ct_event:notify(#event{name=What, node=node(), data=Data}),
            ok;
	tests_done ->
	    ok;
	severe_error ->
